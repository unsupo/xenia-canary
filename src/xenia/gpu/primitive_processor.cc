...
DEFINE_bool(
    ...
    "GPU.Debug");
DEFINE_bool(
    rect_list_vs_expansion, true,
    "Expand guest rectangle-list primitives into triangle strips in the vertex "
    "shader on hosts without geometry shaders (e.g. Vulkan via MoltenVK). "
    "Disable to skip such draws entirely (the pre-expansion behavior) - useful "
    "for A/B testing whether a full-screen rectangle-list pass (tonemap / color "
    "grading / exposure) is responsible for a rendering artifact.",
    "GPU.Debug");
DEFINE_bool(
  ...
  if (tessellation_enabled) {
    ...
    switch (guest_primitive_type) {
      ...
      case xenos::PrimitiveType::kRectangleList:
        if (expand_rectangle_lists_in_vs_ && cvars::rect_list_vs_expansion) {
          host_primitive_type = xenos::PrimitiveType::kTriangleStrip;
          host_vertex_shader_type =
